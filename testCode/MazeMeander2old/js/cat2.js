...
function seekMouse(moved, oneChoice, i){
	if(!moved && mouseLocationX[0] < cat3LocationX[i]){
		var lookAheadTemp = "#" + (cat3LocationX[i]-1)+'-'+cat3LocationY[i];
	...
	if(!moved && mouseLocationY[0] < cat3LocationY[i]){
		var lookAheadTemp = "#" + cat3LocationX[i]+'-'+(cat3LocationY[i]-1);
		if(oneChoice || (lookAheadTemp != oldcat3Location[i]))
			moved = moveCat3Up(i);	
		else{
	...
	for(var i = 0; i < mouseLocation.length; i++){
		...
		if (gameReady && $divTemp.hasClass('cat')){
			if(i==0){
				...
				$divTemp.addClass('mouseSkullBlue');
			}
			else{
				$("#message").html("Red mouse was eaten!");
				$divTemp.addClass('mouseSkullRed');
			}
			deaths++;
		}
		else if($divTemp.hasClass('mouseSkullBlue') || $divTemp.hasClass('mouseSkullBlue'))
			deaths++;
		if(deaths >= mouseLocation.length){
			gameReady = false;
