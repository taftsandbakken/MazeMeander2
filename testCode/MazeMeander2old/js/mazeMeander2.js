...
function checkForPortal(mouseLocationSent, i){
	if (mouseLocationSent == portalLocationRed[0]) {
		if(portalLocationRed.length < 2)
			return;
		var oldMouseLocation = "#" + mouseLocationSent;
	...
	else if (mouseLocationSent == portalLocationRed[1]) {
		if(portalLocationRed.length < 2)
			return;
		var oldMouseLocation = "#" + mouseLocationSent;
		var $oldMouseLocation = $(oldMouseLocation);
	...
	else if (mouseLocationSent == portalLocationBlue[0]) {
	...
	else if (mouseLocationSent == portalLocationBlue[1]) {
		if(portalLocationBlue.length < 2)
			return;
		var oldMouseLocation = "#" + mouseLocationSent;
		var $oldMouseLocation = $(oldMouseLocation);
...
function checkForKey1(mouseLocationSent){
	for (var i = 0; i < key1Location.length; i++) {
		if (mouseLocationSent == key1Location[i]) {
...
function checkSquare(oldMouseLocation2, newMouseLocation2, i){
	...
	checkForKey1(newMouseLocation2);
...
$(document).keydown(function(e){
	...
	if(gameReady){
		if(!paused){
			...
			// second mouse, with the arrow controls
			if(e.which == 37) {
				moveLeft(1);
			}
			if(e.which == 38) {
