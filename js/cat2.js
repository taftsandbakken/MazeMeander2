...
function lookForMouse(randomNum, i){
	...
	var bestChoice = 0;  			//which direction leads to closest mouse
	var recordDistanceToMouse = 0;
	var distanceToMouse = 0;		//counter to measure distance to a mouse
	while(keepLooking){
		distanceToMouse++;
		xTemp--;
		...
		if($lookAheadTemp.hasClass("mouse") && !$lookAheadTemp.hasClass("inaccessibleByCat")){
			canSee2.push(i);
			recordDistanceToMouse = distanceToMouse;
			bestChoice = 1;
		}
	...
	while(keepLooking){
		...
		if($lookAheadTemp.hasClass("mouse") && !$lookAheadTemp.hasClass("inaccessibleByCat")){
			canSee2.push(i);
			if(recordDistanceToMouse == 0 || recordDistanceToMouse > distanceToMouse){
				bestChoice = 2;
				recordDistanceToMouse = distanceToMouse;
			}
		}
...
function seekMouse(moved, oneChoice, i){
	var closestMouse; 			//first, find the closest living mouse
	var totalDistance = new Array();
	for(var j = 0; j < mouseLocation.length; j++){
		if(mousesAlive[j]){
			var xDistance = Math.abs(mouseLocationX[j] - cat3LocationX[i]);
			var yDistance = Math.abs(mouseLocationY[j] - cat3LocationY[i]);
			var combined = xDistance + yDistance;
			totalDistance.push(combined);
			if(closestMouse == null || combined < totalDistance[closestMouse])
				closestMouse = j; 
		}
	}

	
	if(!moved && mouseLocationX[closestMouse] < cat3LocationX[i]){
		var lookAheadTemp = "#" + (cat3LocationX[i]-1)+'-'+cat3LocationY[i];
	...
	if(!moved && mouseLocationY[closestMouse] < cat3LocationY[i]){
		var lookAheadTemp = "#" + cat3LocationX[i]+'-'+(cat3LocationY[i]-1);
		if(oneChoice || (lookAheadTemp != oldcat3Location[i]))
			moved = moveCat3Up(i);	
		else{
...
function checkForCatch(){
	for(var i = 0; i < mouseLocation.length; i++){
		var divTemp = "#" + mouseLocation[i];
		var $divTemp = $(divTemp);
		if (gameReady && $divTemp.hasClass('cat')){
			$divTemp.removeClass("mouse");
			if(i==0){
				$("#message").html("Blue mouse was eaten!");
				$divTemp.addClass('skullBlue');
				mousesAlive[0] = false;
			}
			else{
				$("#message").html("Red mouse was eaten!");
				$divTemp.addClass('skullRed');
				mousesAlive[1] = false;
			}
		}
		var gameOver = true;
		for(var j = 0; j < mousesAlive.length; j++){
			if(mousesAlive[j] == true)
				gameOver = false;
		}
		if(gameOver){
			gameReady = false;
...
function checkSpotToFixPicAsCat($oldLocation){
	...
	if($oldLocation.hasClass("key2"))
		$oldLocation.css("background-image", "url(images/key2.png)");
	if($oldLocation.hasClass("key3"))
		$oldLocation.css("background-image", "url(images/key3.png)");
